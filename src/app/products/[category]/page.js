...
export default function ProductCategoryPage({ params }) {
  ...
  useEffect(() => {
    async function fetchCategory() {
      ...
      // If category is "all" (or common "all-products" slug variants), show all products
      const slug = category.toLowerCase();
      let categoryProductsArr = [];
      if (
        slug === "all" ||
        slug === "all-products" ||
        slug === "all_products" ||
        cat.toLowerCase() === "all" ||
        cat.toLowerCase() === "all products"
      ) {
        // products may be an array or object; Object.values will return an array of product objects
        categoryProductsArr = Object.values(products);
      } else {
        // Fetch the product IDs associated with the category
        const productIds = categoriesData[cat] || [];
        categoryProductsArr = productIds.map((id) => products[id]);
      }

      // Update states after the delay
      setCategoryProducts(categoryProductsArr);
      setFilteredProducts(categoryProductsArr);
      setVisibleProducts(categoryProductsArr.slice(0, itemsPerPage));
    }
          ...
          <div className="w-1/5 p-5 bg-gray-50 rounded-lg filter-bar h-auto max-h-[400px] overflow-x-hidden filter-sidebar hidden xl:block">
            ...
            <div className="flex flex-col gap-2 mt-5 dark:text-black">
              <div className="flex flex-col gap-1 text-base">
                ...
                <Link
                  href="/products/singlePendantLights"
                  className="underline-animation w-max"
                >
                  Single Pendant Lights
                </Link>
                <Link
                  href="/products/landScapeLights"
                  className="underline-animation w-max"
                >
                  Landscape Lights
                </Link>
          ...
          <div className="w-full xl:w-4/5 flex flex-col gap-1 products">
            ...
            <div className=" mt-5 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {loading ? (
                ...
                <div className="flex flex-col items-center col-span-3 justify-center w-full py-20 bg-gray-100 rounded-lg">
